  ...
  const getUserData=async () => {
    try {
      ...
      if(apiResponse.status==200){
        ...
        toast.error(apiResponse.response.data)
      }
    } catch (error) {
       console.log(error)
      toast.error("failed to fetch book data")
        ...
        <div>
          <div className='text-center mt-20'>
            <button
              ...
              className='border p-2 bg-gray-300 text-blue-900 me-10 hover:bg-blue-950 hover:text-white cursor-pointer rounded'
            >
              Books
            </button>
          ...
          {showBooks && 
            <div>
              {bookData?.length > 0 &&
                <div className='grid grid-cols-3 mt-10 mx-2 gap-4'>
          ...
          {showUsers && <div>
            {
              userData.length>0 ? <div className='grid grid-cols-3 gap-4 mt-4'>
                {
                  userData?.map((eachUser)=>(
                    <div className='border bg-amber-500 p-4  '>
                      <div>
                        <img src={`${baseURL}/uploads/${eachUser.proPic}`} alt="" srcset="" />
                      </div>
